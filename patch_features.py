...
cursor_img = None
try:
    cursor_surf = pygame.image.load(get_resource_path("assets/cursor.png")).convert_alpha()
    cursor_img = pygame.transform.scale(cursor_surf, (32, 32))
except:
    # Dummy cursor
    cursor_img = pygame.Surface((20, 20), pygame.SRCALPHA)
    pygame.draw.circle(cursor_img, (255, 0, 0), (10, 10), 5)
    pygame.draw.circle(cursor_img, (0, 0, 0), (10, 10), 2)
pygame.mouse.set_visible(False)

# Sound Loading
# Dummy sounds that silently fail if missing, so the user can just drop them in
class DummySound:
    def play(self): pass
...
try:
    s = pygame.mixer.Sound(get_resource_path("assets/sounds/hit.wav"))
    hit_sound = s
except: pass
try:
    s = pygame.mixer.Sound(get_resource_path("assets/sounds/shoot.wav"))
    shoot_sound = s
except: pass
...
while running:
    ...
    if current_state == STATE_TITLE:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if console_active:
                    if event.key == pygame.K_RETURN:
                        ...
                        if cmd == "/heal":
                            ...
                            is_dev_map = not is_dev_map
                            if is_dev_map:
                                enemies.empty()
                                all_sprites.remove([s for s in all_sprites if isinstance(s, Enemy)])
                                dummy_img = pygame.Surface((150, 150))
                                dummy_img.fill((255, 0, 255))
                                dummy_info = {'image': dummy_img, 'level': 99}
                                dummy = Enemy(player.pos.x + 300, player.pos.y, dummy_info)
                ...
                if is_drawing:
                    # Implement strict drawing size bounds (e.g. 500x500 box restriction)
                    ...
                    if valid_pts:
                        ...
                        # Only allow drawing point if it doesn't expand the box beyond 500px
                        new_x, new_y = event.pos
                        if max(max_x, new_x) - min(min_x, new_x) < 500 and max(max_y, new_y) - min(min_y, new_y) < 500:
                            drawing_points.append(event.pos)
                    else:
                        drawing_points.append(event.pos)
        ...
        spawn_timer += dt
        if spawn_timer >= spawn_rate and not is_dev_map:
            spawn_timer = 0
        ...
        for enemy in enemies:
            ...
            if player.hitbox.colliderect(enemy.hitbox):
                if player.invincibility_timer <= 0:
                    ...
                    hit_sound.play()
                    player.invincibility_timer = 0.5 # 0.5s i-frames
                    if player.health <= 0:
                        current_state = STATE_GAME_OVER
    ...
    for sprite in sorted(all_sprites, key=lambda s: s.rect.bottom):
        # Flicker if player is invincible
        if isinstance(sprite, Player) and sprite.invincibility_timer > 0:
            if int(sprite.invincibility_timer * 10) % 2 == 0:
                continue # Skip drawing this frame to simulate blinking
    ...
    # Draw Console
    if current_state == STATE_PLAYING and console_active:
        console_rect = pygame.Rect(0, HEIGHT - 40, WIDTH, 40)
        pygame.draw.rect(screen, (0, 0, 0, 200), console_rect)
        console_text = font.render("> " + console_input + "_", True, (0, 255, 0))
        screen.blit(console_text, (10, HEIGHT - 35))

    # Draw Custom Cursor (Must be last)
    if cursor_img:
        mx, my = pygame.mouse.get_pos()
        screen.blit(cursor_img, (mx - cursor_img.get_width()//2, my - cursor_img.get_height()//2))
