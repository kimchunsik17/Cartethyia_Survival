...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        # Dynamic Spell Queue Size
        self.max_spell_queue = 3 # Base is 3, can increase to 8
    ...
    def apply_upgrade(self, upgrade_type):
        ...
        elif upgrade_type == "queue_size":
            self.max_spell_queue = min(8, self.max_spell_queue + 1)
...
upgrade_pool = [
    ...
    {"id": "queue_size", "name": "악보 한도 증가", "desc": "작곡 가능한 기호 개수 +1"}
]
...
def create_upgrade_overlay():
    ...
    for upg in upgrade_pool:
        if upg["id"] == "queue_size" and player.max_spell_queue >= 8:
            continue
        valid_upgrades.append(upg)
            
    if len(valid_upgrades) >= 3:
        current_upgrades_offered = random.sample(valid_upgrades, 3)
    else:
        current_upgrades_offered = [random.choice(valid_upgrades) for _ in range(3)]
...
while running:
                ...
                if event.key == pygame.K_LSHIFT:
                    ...
                    if len(valid_points) > 5: # Valid draw
                        ...
                        if spell_name and prob >= 0.50:
                            ...
                            if len(spellbook_queue) < player.max_spell_queue:
                                spellbook_queue.append(spell_name)
                            else:
                                spell_feedback_label = "보관함 가득 참!"
    ...
    if current_state in (STATE_PLAYING, STATE_LEVEL_UP):
        ...
        # Dynamically scale UI base on player's max queue size
        queue_width = 30 + (player.max_spell_queue * 40)
        queue_start_x = WIDTH - queue_width - 20
        queue_start_y = HEIGHT - 80
        pygame.draw.rect(screen, (30, 30, 30), (queue_start_x, queue_start_y, queue_width, 60))
        ...
        pygame.draw.rect(screen, border_color, (queue_start_x, queue_start_y, queue_width, 60), 2)
        
        try:
            state_str = "실행 중" if len(active_score) > 0 else f"보관함 ({len(display_queue)}/{player.max_spell_queue})"
            ...
            # Draw empty placeholder slots up to player.max_spell_queue if it's the spelling queue
            if len(active_score) == 0:
                for i in range(player.max_spell_queue):
                    cx = queue_start_x + 30 + (i*40)
                    cy = queue_start_y + 30
                    pygame.draw.circle(screen, (40, 40, 40), (cx, cy), 18)
                    pygame.draw.circle(screen, (80, 80, 80), (cx, cy), 18, 1)
